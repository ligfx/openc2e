...
namespace ImGuiSDL
{
	...
	bool NextPrimitiveIsRectangle(ImVector<ImDrawVert> vertexBuffer, unsigned short* indexBuffer, unsigned int i, ImVec2 *top_left_uv, ImVec2 *bottom_right_uv) {
		...
		const float min_x = SDL_min(v0.pos.x, SDL_min(v1.pos.x, v2.pos.x));
		const float max_x = SDL_max(v0.pos.x, SDL_max(v1.pos.x, v2.pos.x));
		const float min_y = SDL_min(v0.pos.y, SDL_min(v1.pos.y, v2.pos.y));
		const float max_y = SDL_max(v0.pos.y, SDL_max(v1.pos.y, v2.pos.y));

		// All vertices must have the same color - gradients must go through the rasterizer
		if (!(v0.col == v1.col && v1.col == v2.col && v2.col == v3.col
		      && v3.col == v4.col && v4.col == v5.col)) {
			return false;
		}

		// All vertices must be on the corners of the original bounding box
		// - we only handle non-rotated rectangles
		// Additionally, keep track of where each vertex is on the bounding box
		ImDrawVert *top_left = NULL;
		ImDrawVert *top_right = NULL;
		ImDrawVert *bottom_left = NULL;
		ImDrawVert *bottom_right = NULL;
		for (int j = 0; j < 3; ++j) {
			ImDrawVert *v = &vertexBuffer[indexBuffer[i + j]];
			if (v->pos.x == min_x && v->pos.y == min_y) top_left = v;
			else if (v->pos.x == max_x && v->pos.y == min_y) top_right = v;
			else if (v->pos.x == min_x && v->pos.y == max_y) bottom_left = v;
			else if (v->pos.x == max_x && v->pos.y == max_y) bottom_right = v;
			else return false;
		}
		ImDrawVert *second_top_left = NULL;
		ImDrawVert *second_top_right = NULL;
		ImDrawVert *second_bottom_left = NULL;
		ImDrawVert *second_bottom_right = NULL;
		for (int j = 3; j < 6; ++j) {
			ImDrawVert *v = &vertexBuffer[indexBuffer[i + j]];
			if (v->pos.x == min_x && v->pos.y == min_y) second_top_left = v;
			else if (v->pos.x == max_x && v->pos.y == min_y) second_top_right = v;
			else if (v->pos.x == min_x && v->pos.y == max_y) second_bottom_left = v;
			else if (v->pos.x == max_x && v->pos.y == max_y) second_bottom_right = v;
			else return false;
		}

		// Make sure that the two triangles fill opposite halves of the bounding box
		if (top_left == NULL && second_bottom_right != NULL) return false;
		else if (top_right == NULL && second_bottom_left != NULL) return false;
		else if (bottom_left == NULL && second_top_right != NULL) return false;
		else if (bottom_right == NULL && second_top_left != NULL) return false;

		// Make sure that the UVs at each overlapping point match
		if (top_left && second_top_left && top_left->uv != second_top_left->uv) return false;
		if (top_right && second_top_right && top_right->uv != second_top_right->uv) return false;
		if (bottom_left && second_bottom_left && bottom_left->uv != second_bottom_left->uv) return false;
		if (bottom_right && second_bottom_right && bottom_right->uv != second_bottom_right->uv) return false;

		// Copy missing fourth point onto the first rectangle for ease
		if (!top_left) top_left = second_top_left;
		else if (!top_right) top_right = second_top_right;
		else if (!bottom_left) bottom_left = second_bottom_left;
		else if (!bottom_right) bottom_right = second_bottom_right;

		// Make sure UVs were originally derived from a rectangle
		if (top_left->uv + bottom_right->uv != top_right->uv + bottom_left->uv) return false;

		*top_left_uv = top_left->uv;
		*bottom_right_uv = bottom_right->uv;
			...
			for (int cmd_i = 0; cmd_i < commandList->CmdBuffer.Size; cmd_i++)
			{
				...
				else
				{
					...
					for (unsigned int i = 0; i + 3 <= drawCommand->ElemCount; i += 3)
					{
						...
						ImVec2 top_left_uv;
						ImVec2 bottom_right_uv;
						bool isRectangle = i + 6 <= drawCommand->ElemCount && NextPrimitiveIsRectangle(vertexBuffer, indexBuffer, i, &top_left_uv, &bottom_right_uv);
						if (isRectangle)
						{
							const bool doHorizontalFlip = bottom_right_uv.x < top_left_uv.x;
							const bool doVerticalFlip = bottom_right_uv.y < top_left_uv.y;
