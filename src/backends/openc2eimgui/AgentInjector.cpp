...
void DrawAgentInjector() {
	if (Openc2eImgui::BeginWindow("Agent Injector", &s_agent_injector_open)) {
		if (!s_read_agents) {
		...
		{
			Openc2eImgui::SetNextStretchMinSize(ImVec2(300, 100));
			ImGui::BeginChild("Objects", Openc2eImgui::GetStretchSize(), true);
		...
		if (s_object_selected_index < s_object_injector.objects.size() && s_object_injector.objects[s_object_selected_index].is_removable) {
			...
			Openc2eImgui::DisabledButton("Remove Object");
		}
