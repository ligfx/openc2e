...
void DrawC2StatusBar() {
  ...
  if (ImGui::Begin("Statusbar", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse)) {
    ...
    if (world.season < s_season_icons_texs.size()) {
      Openc2eImgui::Image(s_season_icons_texs[world.season]);
    }
    ...
    if (world.timeofday < s_timeofday_icons_texs.size()) {
      Openc2eImgui::Image(s_timeofday_icons_texs[world.timeofday]);
    }
    ...
    if (temperature) {
      ...
      if (tempiconid < s_temperature_icons_texs.size()) {
        Openc2eImgui::Image(s_temperature_icons_texs[tempiconid]);
      }
    ...
    if (!s_healthicons_texs[0]) {
      s_healthicons_texs[0] = GetTextureWithTransparencyFromExeFile(0xe8); // disabled (gray)
      s_healthicons_texs[1] = GetTextureWithTransparencyFromExeFile(0xc7); // 0/4
      s_healthicons_texs[2] = GetTextureWithTransparencyFromExeFile(0xac); // 1/4
      s_healthicons_texs[3] = GetTextureWithTransparencyFromExeFile(0xad); // 2/4
      s_healthicons_texs[4] = GetTextureWithTransparencyFromExeFile(0xab); // 3/4
      s_healthicons_texs[5] = GetTextureWithTransparencyFromExeFile(0xaa); // 4/4
    }
    
    static std::array<Texture, 5> s_hearticons_texs;
    if (!s_hearticons_texs[0]) {
      s_hearticons_texs[0] = GetTextureWithTransparencyFromExeFile(0xe9); // disabled (gray)
      s_hearticons_texs[1] = GetTextureWithTransparencyFromExeFile(0xae); // large
      s_hearticons_texs[2] = GetTextureWithTransparencyFromExeFile(0xaf); // medium
      s_hearticons_texs[3] = GetTextureWithTransparencyFromExeFile(0xb0); // small
      s_hearticons_texs[4] = GetTextureWithTransparencyFromExeFile(0xe7); // dead (blue)
    }
    ...
    if (!world.selectedcreature) {
      Openc2eImgui::Image(s_healthicons_texs[0]);
      Openc2eImgui::Image(s_hearticons_texs[0]);
    } else {
      if (CreatureAgent *ca = dynamic_cast<CreatureAgent *>(world.selectedcreature.get())) {
        ...
        if (c->isAlive()) {
          ...
          Openc2eImgui::Image(s_healthicons_texs[x+2]); // 2-5 (1-4 bars)
          // TODO: flash indicator when norn is dying

          // TODO: heartbeat animation
          Openc2eImgui::Image(s_hearticons_texs[1]); // 1-3 = beating heart, large to small
        } else {
          // dead
          Openc2eImgui::Image(s_healthicons_texs[1]);
          Openc2eImgui::Image(s_hearticons_texs[4]); // blue heart
        }
        // TODO: drive
      }
    }
