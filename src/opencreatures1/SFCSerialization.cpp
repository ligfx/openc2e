...
static bool create_engine_object(SFCLoader* ctx, sfc::ObjectV1* p) {
	...
	static_cast<U*>(obj)->load(*ctx, static_cast<T*>(p));
	return true;
}
...
template <typename T, typename U>
bool dump_engine_object(SFCSaver* ctx, Object* p) {
	if (typeid(*p) != typeid(U)) {
		return false;
	}
	auto obj = std::make_shared<T>();
	ctx->sfc_object_mapping[p] = obj;
	static_cast<U*>(p)->save(*ctx, static_cast<T*>(obj.get()));
...
static void sfc_dump_objects_and_sceneries(SFCSaver& ctx) {
