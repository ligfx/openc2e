...
void Vehicle::load(SFCLoader& ctx, const sfc::VehicleV1* veh) {
	...
	static_cast<CompoundObject*>(this)->load(ctx, static_cast<const sfc::CompoundObjectV1*>(veh));
...
void Vehicle::save(SFCSaver& ctx, sfc::VehicleV1* veh) const {
	// recurse before the Vehicle logic (unlike the other serialization functions) because
	// we need to adjust the parts' position based on the vehicle's fixed point 24.8 position.
	static_cast<const CompoundObject*>(this)->save(ctx, static_cast<sfc::CompoundObjectV1*>(veh));
